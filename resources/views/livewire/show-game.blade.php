<div wire:init='loadGame' class="container mx-auto px-4">
    @push('scripts')
        ...
        <x-rating-progress-bar event="aggregatedCritics" />
        <x-rating-progress-bar event="similarGame" />
    @endpush
    @if ($game)
            ...
            <div class="ml-0 lg:ml-12 lg:mr-64 mr-0">
                ...
                <div class="flex flex-wrap items-center mt-8">
                    <div class="flex items-center">
                        <div class="w-16 h-16 bg-gray-800 rounded-full relative">
                            <div id="memberCritics" class="font-semibold text-xs absolute"></div>
                        </div>
                        <div class="ml-4 text-sm font-bold">Member <br> Score</div>
                    </div>
                    <div class="flex items-center ml-12">
                        <div class="w-16 h-16 bg-gray-800 rounded-full relative">
                            <div id="aggregatedCritics" class="font-semibold text-xs absolute"></div>
                        </div>
                        <div class="ml-4 text-sm font-bold">Critics <br> Score</div>
                    </div>
                    <div class="flex items-center space-x-4 mt-6 lg:mt-0 lg:ml-12 ml-0">
                        @isset($game['socials']['website'])
                            <div class="rounded-full bg-gray-800 w-8 h-8 flex justify-center items-center">
                                <a href="{{$game['socials']['website']}}" class="hover:text-gray-400" target="_blanck">
                        ...
                        @endisset
                        @isset($game['socials']['instagram'])
                            <div class="rounded-full bg-gray-800 w-8 h-8 flex justify-center items-center">
                                <a href="{{$game['socials']['instagram']}}" class="hover:text-gray-400" target="_blanck">
                        ...
                        @endisset
                        @isset($game['socials']['twitter'])
                            <div class="rounded-full bg-gray-800 w-8 h-8 flex justify-center items-center">
                                <a href="{{$game['socials']['twitter']}}" class="hover:text-gray-400" target="_blanck">
                        ...
                        @endisset
                        @isset($game['socials']['facebook'])
                            <div class="rounded-full bg-gray-800 w-8 h-8 flex justify-center items-center">
                                <a href="{{$game['socials']['facebook']}}" class="hover:text-gray-400" target="_blanck">
    ...
    @else
        <div class="game-details border-b border-gray-800 pb-12 flex flex-col lg:flex-row animate-pulse">
            <div class="flex-none">
                <div class="h-64 w-56 bg-gray-800 rounded"> </div>
            </div>
            <div class="ml-0 lg:ml-12 lg:mr-64 mr-0">
                <h2 class="text-transparent rounded bg-gray-800">The magnificent name of that game</h2>
                ...
                <div class="flex flex-wrap items-center mt-8">
                    <div class="flex items-center">
                        <div class="h-16 w-16 bg-gray-800 rounded-full"></div>
                    </div>
                    <div class="flex items-center ml-12">
                        <div class="h-16 w-16 bg-gray-800 rounded-full"></div>
                    </div>
                    ...
                    <div class="flex flex-col">
                        <p class="lg:mt-12 mt-4 rounded bg-gray-800 text-transparent">
                            Lorem ipsum, dolor sit amet consectetur adipisicing elit. Quod nobis vel odit eligendi voluptatibus consequatur quaerat quasi aut repellendu
                        </p>
                    </div>
                </div>
            </div>
        </div>
        <div class="image-container border-b border-gray-800 pb-12 mt-8 animate-pulse">
            <h2 class="text-blue-500 uppercase tracking-wide font-bold">
                Images
            </h2>
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12 mt-8">
                @foreach (range(1,9) as $screenshot)
                    <div>
                        <div class="w-64 h-44 rounded bg-gray-800"> </div>
                    </div>
                @endforeach
            </div>
        </div>
